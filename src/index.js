...
export default class OctoAuthClient{
    constructor({clientId, redirectURI, scopes, serverURL}){
        ...
        this.session = new HTTPSession({baseURL: this.serverURL});
    ...
    setTokenGrant({access_token, refresh_token, token_type, expires_in}){
        ...
        if(token_type == "bearer"){
            this.session.updateHeaders({"Authorization": `${token_type} ${access_token}`});
    ...
    async reloadAuthorization(){
        ...
        if(!refreshToken){
            return this.isAuthorized.next(false);
        }
    ...
    async getCurrentUserAccount(){
        const response = await this.session.get({url: "/accounts/whoami"});
        if(response.status != 200){
